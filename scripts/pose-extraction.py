...
def main(camera_id, hrnet_c, hrnet_j, hrnet_weights, hrnet_joints_set, image_resolution, disable_tracking,
    ...
    data_folder = "/media/lab/Z/parkinson/raw_data/data_03_19/BW/*.avi"
    output_folder = "/media/lab/Z/parkinson/visual/visual_03_19/BW/"
    out_dict_folder ='/media/lab/Z/parkinson/keypts/keypoints_03_19/BW/'
    try: os.makedirs(output_folder)
    except: pass
    try: os.makedirs(out_dict_folder)
    except: pass
    vid_list = glob(data_folder)
    print("start from{0} to {1}".format(start,end))

    for vid_path in tqdm(vid_list[start:end]):

        out_path = output_folder+vid_path.split("/")[-1]
        out_dict = out_dict_folder+vid_path.split("/")[-1][:-4]
        ...
        for i in (range(int(video.get(7)))):
            ...
            if vid_path is not None or disable_vidgear:
                ...
                if len(pts) > 0:
                    if prev_pts is None and prev_person_ids is None:
                        ...
                        boxes, pts, person_ids = find_person_id_associations(
                            ...
                            next_person_id=next_person_id, pose_alpha=0.2, similarity_threshold=0.25, smoothing_alpha=0.4,
                        )
            ...
            if save_video:
                if video_writer is None:
                    ...
                    video_writer = cv2.VideoWriter(out_path, fourcc, video_framerate, (frame.shape[1], frame.shape[0]))
                video_writer.write(frame)
            frame_count += 1

        if extract_pts:
            np.save(out_dict, pts_dict)
        if save_video:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start", help="start_time", type=int, default=0)
    parser.add_argument("--end", help="end_time", type=int, default=300)
